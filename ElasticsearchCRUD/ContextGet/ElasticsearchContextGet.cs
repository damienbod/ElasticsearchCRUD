	...
	public class ElasticsearchContextGet
	{
		...
		public bool DocumentExists<T>(object entityId, object parentId)
		{
			try
			{
				...
				if (task.Result.Status == HttpStatusCode.NotFound)
				{
					_traceProvider.Trace(TraceEventType.Information, "ElasticSearchContextGet: DocumentExists HttpStatusCode.NotFound");
				}
			...
			catch (AggregateException ae)
			{
				ae.Handle(x =>
				{
					_traceProvider.Trace(TraceEventType.Warning, x, "{2} DocumentExists {0}, {1}", typeof(T), entityId, "ElasticSearchContextGet");
					if (x is ElasticsearchCrudException || x is HttpRequestException)
					{
						throw x;
					}
			...
			_traceProvider.Trace(TraceEventType.Error, "{2}: Unknown error for DocumentExists {0}, Type {1}", entityId, typeof(T), "ElasticSearchContextGet");
			throw new ElasticsearchCrudException(string.Format("{2}: Unknown error for DocumentExists {0}, Type {1}", entityId, typeof(T), "ElasticSearchContextGet"));
		}

		//	 HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Head, new Uri("http://localhost:9200"));
		//	 HttpResponseMessage response = await httpClient.SendAsync(request);
		public async Task<ResultDetails<bool>> DocumentExistsAsync<T>(object entityId, object parentId)
		{
			_traceProvider.Trace(TraceEventType.Verbose, "{2}: Request for exist document with id: {0}, Type: {1}", entityId, typeof(T), "ElasticSearchContextGet");
			...
			try
			{
				...
				if (response.StatusCode != HttpStatusCode.OK)
				{
					...
					_traceProvider.Trace(TraceEventType.Information, "{2}: DocumentExistsAsync response status code: {0}, {1}", response.StatusCode, response.ReasonPhrase, "ElasticSearchContextGet");
