...
namespace ElasticsearchCRUD.ContextSearch
{
	public class ElasticsearchContextSearch
	{
		...
		public async Task<ResultDetails<Collection<T>>> SearchForChildDocumentsByParentIdAsync<T>(object parentId, string parentDocumentType)
		{
			...
			var index = elasticSearchMapping.GetIndexForType(typeof(T));
			var type = elasticSearchMapping.GetDocumentType(typeof(T));
			_traceProvider.Trace(TraceEventType.Verbose, string.Format("ElasticsearchContextSearch: Searching for child documents with parent id: {0}, index {1}, type {2}", parentId, index, type));
			...
			{
				resultDetails.Status = HttpStatusCode.NotFound;
				resultDetails.Description = string.Format("No child documents found parent id: {0}, index {1}, type {2}", parentId, index, type);
				_traceProvider.Trace(TraceEventType.Information, string.Format("ElasticsearchContextSearch: No child documents found parent id: {0},, index {1}, type {2}", parentId, index, type));
			...
			resultDetails.Status = result.Status;
			resultDetails.Description = result.Description;
			_traceProvider.Trace(TraceEventType.Error, string.Format("ElasticsearchContextSearch: No child documents found parent id: {0},  index {1}, type {2}", parentId, index, type));
			return resultDetails;
		}
		...
		public Collection<T> SearchForChildDocumentsByParentId<T>(object parentId, string parentDocumentType)
		{
			try
			{
				Task<ResultDetails<Collection<T>>> task = Task.Run(() => SearchForChildDocumentsByParentIdAsync<T>(parentId, parentDocumentType));
				task.Wait();
				if (task.Result.Status == HttpStatusCode.NotFound)
				{
					_traceProvider.Trace(TraceEventType.Warning, "ElasticsearchContextSearch: HttpStatusCode.NotFound");
			...
			catch (AggregateException ae)
			{
				ae.Handle(x =>
				{
					_traceProvider.Trace(TraceEventType.Warning, x, "{2} SearchForChildDocumentsByParentId {0}, {1}", typeof(T), parentId, "ElasticsearchContextSearch");
					if (x is ElasticsearchCrudException || x is HttpRequestException)
					{
						throw x;
					}
			...
			_traceProvider.Trace(TraceEventType.Error, "{2}: Unknown error for SearchForChildDocumentsByParentId {0}, Type {1}", parentId, typeof(T), "ElasticsearchContextSearch");
			throw new ElasticsearchCrudException(string.Format("{2}: Unknown error for SearchForChildDocumentsByParentId {0}, Type {1}", parentId, typeof(T), "ElasticsearchContextSearch"));
		}
